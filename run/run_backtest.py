...
def main() -> None:
    ...
    if cfg["strategy"]["use_chanlun"]:
        price_feat = price_feat.groupby("instId", group_keys=False).apply(detect_chanlun_state).reset_index(drop=True)
    sig = build_strategy_signals(price_feat, factors, cfg["strategy"]["nbi_long_quantile"], cfg["strategy"]["nbi_short_quantile"], cfg["strategy"]["consensus_burst_quantile"])
    sig = apply_risk_controls(sig, cfg["backtest"]["risk_per_trade"], cfg["backtest"]["max_leverage"], cfg["backtest"]["max_daily_loss"], cfg["backtest"]["max_drawdown"])
    exe = simulate_execution(sig)
    port, metrics = run_backtest(exe, cfg["backtest"]["initial_nav"], cfg["backtest"]["taker_fee_bps"], cfg["backtest"]["slippage_bps"], out_dir)
    build_monitor_snapshot(price_feat, factors, out_dir)

    with open(Path(out_dir) / "backtest_metrics.json", "w", encoding="utf-8") as f:
